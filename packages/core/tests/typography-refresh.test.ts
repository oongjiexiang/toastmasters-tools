 ...
 * exact pre-Phase-29 file (must NOT hold). Whitespace-insensitive so a
...
describe("negative control: pre-Phase-29 heading sites are rejected", () => {
  ...
  // no tracking-tight.
  const preP29DashboardHeaderSource =
  ...
  it("fails for DashboardHeader's pre-Phase-29 <h1> (no tracking-tight)", () => {
    const className = extractH1ClassName(preP29DashboardHeaderSource);
    expect(() =>
      assertClassTokensPresent(className, ["text-2xl", "font-semibold", "tracking-tight"]),
    ).toThrow();
  });
  ...
  it("fails for MemberDetailView's pre-Phase-29 <h1> (no tracking-tight)", () => {
    const className = extractH1ClassName(preP29MemberDetailViewSource);
    expect(() =>
      assertClassTokensPresent(className, ["text-2xl", "font-semibold", "tracking-tight"]),
    ).toThrow();
  });
  ...
  it("fails for CardTitle's pre-Phase-29 className (no tracking-tight)", () => {
    const className = extractCardTitleBaseClassName(preP29CardTitleSource);
  ...
  it("still finds the size classes present in the pre-Phase-29 fixtures (proves these aren't just any old string)", () => {
    ...
    expect(extractCardTitleBaseClassName(preP29CardTitleSource)).toContain(
