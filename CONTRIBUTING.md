...
3. Once a PR merges, `main` is always green (per (2)) and several things happen automatically —
   ...
   - `ci.yml` has already run the full test suite (core + desktop) **and** a workspace typecheck
     gate (`npm run typecheck --workspaces`, no `--if-present` — a workspace missing the script
     hard-fails rather than being silently skipped) on the PR itself, both inside the same `test`
     job so the single required `test` status context covers both.
