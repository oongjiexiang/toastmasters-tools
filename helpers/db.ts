...
export function printProgressDiff(dbPath = DEFAULT_DB_PATH): void {
  const result = getProgressDiff(dbPath);

  if (!result) {
    console.log("Progress: only one snapshot found — run fetch again after changes to see a diff.");
    return;
  }

  console.log(`\n=== Progress diff: ${fmtDate(result.older)} → ${fmtDate(result.newer)} ===\n`);

  if (result.changes.length === 0) {
    console.log("  No level changes.\n");
  } else {
    for (const c of result.changes) {
      console.log(`  ${c.firstName} ${c.lastName} (${c.pathName}): gained ${c.gained.join(", ")}`);
    }
    console.log();
  }
}

export function printMembershipDiff(dbPath = DEFAULT_DB_PATH): void {
  const result = getMembershipDiff(dbPath);

  if (!result) {
    console.log("Membership: only one snapshot found — run membership again after changes to see a diff.");
    return;
  }

  console.log(`=== Membership diff: ${fmtDate(result.older)} → ${fmtDate(result.newer)} ===\n`);

  if (result.joined.length === 0 && result.left.length === 0 && result.statusChanged.length === 0) {
    console.log("  No membership changes.\n");
  } else {
    if (result.joined.length > 0) {
      console.log("  Joined / renewed:");
      result.joined.forEach(m => console.log(`    ${m.name}`));
      console.log();
    }
    if (result.left.length > 0) {
      console.log("  Left / not in new export:");
      result.left.forEach(m => console.log(`    ${m.name}`));
      console.log();
    }
    if (result.statusChanged.length > 0) {
      console.log("  Status changed:");
      result.statusChanged.forEach(m =>
        console.log(`    ${m.name}: ${m.oldStatus} → ${m.newStatus}`)
      );
      console.log();
    }
  }
