...
- **Log in, no cookie pasting:** click **Log in** and sign in on the embedded Toastmasters
  ...
  its own `config.env`), and stays logged in across restarts until the session expires. The login
...
- **`ci.yml`** runs the full test suite (`npm test` — core + desktop) **and** a workspace
  typecheck gate (`npm run typecheck --workspaces`, deliberately without `--if-present` so a
  future workspace with no `typecheck` script hard-fails instead of being silently skipped) in
  the same `test` job, on every branch push and PR into `main`. The typecheck step catches `tsc`
  errors that `npm test`'s per-file vitest transpilation can miss. The root `package.json` pins
  `typescript` in both `devDependencies` and `overrides`, forcing every resolution in the tree —
  every workspace, every transitive dependency — onto that major, so a dependency reshuffle can't
  silently reintroduce the Phase 23 regression (an unpinned workspace inheriting a hoisted
  `typescript@6.x`). Neither step needs Toastmasters cookies — the tests mock the network.
